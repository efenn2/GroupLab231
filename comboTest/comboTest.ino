...
void loop() {
  ...
  if ((mode == 1) && (digitalRead(8) == 0)) {
    ...
    if (now - lastButtonPress > DEBOUNCE_TIME) {
      lastButtonPress = now;
      handleRightButtonPress(message, lastMessage, defaultMessage);
    } else {
      now = millis();
...
void responsiveMessageWithoutInterrupts(unsigned long now, uint8_t *message, uint8_t defaultMessage[8], uint8_t *lastMessage) {
  if (now - lastButtonPress > DEBOUNCE_TIME) {
    lastButtonPress = now;
    handleRightButtonPress(message, lastMessage, defaultMessage);
  } else {
...
void handleRightButtonPress(uint8_t *message, uint8_t *lastMessage, uint8_t defaultMessage[8]) {
  if (digitalRead(9) != 1) {
...
uint8_t *tempMessage = NULL;
uint8_t *tempLast = NULL;
uint8_t tempDefault[8] = {0, 0, 0x1, 0, 0, 0x1, 0, 0};;
void changingMode() {
  //tempMessage = message;
  // int count = 8;
  // for (int x = 0; x < 8; x++) {
  ...
  tempMessage = tempDefault;

  //  for (int i = 0; i< 8; i++) {
  //    Serial.print(defaultMessage[i]);
  //    Serial.print(" ,  ");
  //    Serial.print(leftCursor[i]);
  //    Serial.print(" ,  ");
  ...
  while (digitalRead(8) != 0) {
    ...
    if (now - lastButtonPress > DEBOUNCE_TIME) {
      lastButtonPress = now;
      handleRightButtonPress(tempMessage, tempLast, tempDefault);
    } else {
      now = millis();
      if (now - countdownStart > 1000) {
        countdownStart = now;
        if (tempMessage == tempDefault) {
          Serial.println("Message update 1 Changing");
          tempMessage = tempLast;
          tempLast = tempDefault;
        } else {
          Serial.println("Message update 2 Changing");
          tempLast = tempMessage;
          tempMessage = tempDefault;
        }
        if (tempMessage != NULL) {
